...
const Arrow = ({ className, style, onClick, direction }) => (
  <div 
    ...
    style={{
      ...
      position: 'absolute', 
      top: '50%', 
      [direction]: '25px', // Further out to the side
      transform: 'translateY(-50%)', // Center vertically
      cursor: 'pointer',
      fontSize: '30px', // Bigger size for arrow
    }} 
    onClick={onClick}
  >
    {direction === 'right' ? '>' : '<'} {/* Big arrow symbols */}
  </div>
);
...
const ImageLayout = () => {
    ...
    const carouselSettings = {
        ...
        slidesToShow: 3,
        slidesToScroll: 1,
        nextArrow: <Arrow direction="right" />, // Custom next arrow on the right
        prevArrow: <Arrow direction="left" />,  // Custom prev arrow on the left
        responsive: [
            {
              breakpoint: 1024,
              settings: {
